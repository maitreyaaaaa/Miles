...
const SIZES = {
  ...
  md: { height: 38, font: 13, pad: 16, min: 44 },
...
export default function RubberSegment({
  ...
  useLayoutEffect(() => {
    ...
    if (typeof document !== 'undefined' && (document as any).fonts) (document as any).fonts.ready.then(measure);
  ...
  const land = (to: number, v: number | null, flick: boolean, withSquash: boolean) => {
    ...
    if (!withSquash || squash <= 0) {
      animate(trail, trailTo, { ...SPRING_UI, duration: t(0.3), velocity: trailVelocity } as any);
      return;
    }
    animate(trail, trailTo + dir * squash, { ...SPRING_UI, duration: t(0.3), velocity: trailVelocity } as any).then(() => {
      if (gen.current === g) animate(trail, trailTo, { ...SPRING_RELAX, duration: t(0.16) } as any);
    });
  };
  ...
  const travel = (from: number, to: number) => {
    ...
    animate(edgeL, b.l + (Math.min(a.l, b.l) - b.l) * u, tween as any);
    animate(edgeR, b.r + (Math.max(a.r, b.r) - b.r) * u, tween as any);
  ...
  const handlePointerDown = (e: React.PointerEvent<HTMLButtonElement>, i: number) => {
    ...
    box.current = trackRef.current ? trackRef.current.getBoundingClientRect() : null;
    try {
  ...
  return (
    <div
      ...
      style={{
        ['--rs-track' as any]: trackColor,
        ['--rs-thumb' as any]: thumbColor,
        ['--rs-ink' as any]: textColor,
        ['--rs-ink-active' as any]: activeTextColor,
        ['--rs-radius' as any]: `${radius}px`,
        ['--rs-inset' as any]: `${inset}px`,
        ['--rs-thumb-radius' as any]: `${thumbRadius}px`,
        ['--rs-h' as any]: `${preset.height}px`,
        ['--rs-font' as any]: `${preset.font}px`,
        ['--rs-pad' as any]: `${preset.pad}px`,
        ['--rs-min' as any]: `${preset.min}px`
      }}
